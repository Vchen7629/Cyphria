...
class NormalizerFactory:
    ...
    @staticmethod
    def normalize(product_topic: str, product_list: list[str], logger: Optional[StructuredLogger] = None) -> Optional[list[str]]:
        """
        ...
        if not product_topic or not product_list or product_topic.strip() == "":
            if logger:
                logger.error(event_type="ingestion_service run", message="Missing product topic, can't normalize")
            return None
