...
def test_remove_duplicate_gpu_name():
    ...
    result = GPUNameNormalizer().normalize_product_list(detected_products)

    assert result == ["NVIDIA RTX 4090"]

def test_normalizes_no_spaces_gpu_name():
...
def test_normalizes_case_insensitive_gpu_name():
...
def test_founders_edition_gpu_normalized():
...
def test_empty_list():
    ...
    result = GPUNameNormalizer().normalize_product_list([])

    assert result == []

def test_all_invalid_gpu_names():
...
def test_unknown_model_numbers():
