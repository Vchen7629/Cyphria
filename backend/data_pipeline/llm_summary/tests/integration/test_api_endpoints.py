...
def test_run_endpoint_success(fastapi_client: FastAPITestClient) -> None:
    ...
    with patch('src.api.routes.LLMSummaryWorker') as MockService:
        mock_instance = MagicMock()
        mock_instance.run.return_value = SummaryResult(
            products_summarized=100,
            cancelled=False
        )
        ...
        assert data["result"]["products_summarized"] == 100
...
def test_run_endpoint_error_handling(fastapi_client: FastAPITestClient) -> None:
    ...
    with patch('src.api.routes.LLMSummaryWorker') as MockService:
        mock_instance = MagicMock()
        mock_instance.run.side_effect = Exception("Unknown Error")
        MockService.return_value = mock_instance
