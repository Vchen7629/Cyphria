...
def test_fetch_single_product(db_connection: psycopg.Connection, single_product_sentiment: dict[str, Any]) -> None:
...
def test_fetch_product_with_less_than_min_comments(db_connection: psycopg.Connection, single_product_sentiment: dict[str, Any]) -> None:
...
def test_fetch_orders_by_comment_count_desc(db_connection: psycopg.Connection, single_product_sentiment: dict[str, Any]) -> None:
...
def test_fetch_only_last_90_day(db_connection: psycopg.Connection, single_product_sentiment: dict[str, Any]) -> None:
    """90 day time_window should fetch only products that have comments from last 90 days"""
    products_with_counts = [
        ("rtx 5090", datetime.now(timezone.utc) - timedelta(days=5), 11), # rtx 5090 is valid since its only 5 days old
        ("rtx 5080", datetime.now(timezone.utc) - timedelta(days=300), 16), # rtx 5080 is invalid since its 300 days old
    ]

    product_sentiments = [
        {**single_product_sentiment, "product_name": name, "created_utc": created_utc, "comment_id": f"test_comment_{name}_{i}"}
...
def test_time_window_90_day_border(db_connection: psycopg.Connection, single_product_sentiment: dict[str, Any]) -> None:
    ...
    products_with_counts = [
        ...
        ("rtx 5080", datetime.now(timezone.utc) - timedelta(days=89, hours=23, minutes=59, seconds=59, milliseconds=999), 16),
    ]
