...
def test_empty_input_paramaters(mock_db_connection: MagicMock) -> None:
    ...
    empty_product_name_res = fetch_top_comments_for_product(mock_db_connection, product_name="", time_window="all_time")
    assert len(empty_product_name_res) == 0
    assert empty_product_name_res == []

    empty_time_window_res = fetch_top_comments_for_product(mock_db_connection, product_name="rtx 4090", time_window="")
...
def test_none_input_paramaters(mock_db_connection: MagicMock) -> None:
    ...
    none_product_name_res = fetch_top_comments_for_product(mock_db_connection, product_name=None, time_window="all_time") # type: ignore
    assert len(none_product_name_res) == 0
    assert none_product_name_res == []

    none_time_window_res = fetch_top_comments_for_product(mock_db_connection, product_name="rtx 4090", time_window=None) # type: ignore
