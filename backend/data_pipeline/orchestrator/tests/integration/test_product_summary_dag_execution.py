...
def test_dag_pipeline_full_execution(
    ...
    assert task_states.get("generate_product_summaries_all_time") == "success"
    assert task_states.get("wait_generate_product_summaries_all_time") == "success"
    assert task_states.get("generate_product_summaries_90_day") == "success"
    assert task_states.get("wait_generate_product_summaries_90_day") == "success"
...
def test_dag_downstream_task_skipped_on_upstream_failure(
    ...
    """wait all_time task is skipped/failed if all_time fails."""
    configure_mock_service(
        k8s_core_api, test_namespace, "llm-summary-service",
        should_fail=True
    )
    ...
    task_states = get_task_states(k8s_core_api, test_namespace, airflow_pod, dag_id, run_id)
    assert task_states.get("generate_product_summaries_all_time") == "failed"
    # Downstream tasks should not succeed
    assert task_states.get("wait_generate_product_summaries_all_time") in ("upstream_failed", "skipped", None)
...
def test_on_failure_callback_invoked(
    ...
    logs = exec_airflow_cmd(
        ...
        ["airflow", "tasks", "logs", dag_id, "generate_product_summaries_all_time", run_id]
    )
