...
def test_sentiment_analysis_dag_pipeline_full_execution(
    ...
    assert task_states.get("ingest_computing_comments") == "success"
    assert task_states.get("wait_ingest_computing_comments") == "success"
    assert task_states.get("analyze_computing_product_sentiments") == "success"
    assert task_states.get("wait_analyze_computing_product_sentiments") == "success"
...
def test_downstream_tasks_skipped_on_upstream_failure(
    ...
    task_states = get_task_states(k8s_core_api, test_namespace, airflow_pod, dag_id, run_id)
    assert task_states.get("ingest_computing_comments") == "failed"
    # Downstream tasks should not succeed
    assert task_states.get("wait_ingest_computing_comments") in ("upstream_failed", "skipped", None)
