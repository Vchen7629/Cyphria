...
def test_upsert_one_product_score(
    db_connection: psycopg.Connection, single_product_score_comment: ProductScore
) -> None:
    ...
    with db_connection.cursor() as cursor:
        cursor.execute(
            "SELECT product_name FROM product_rankings WHERE product_topic = %s;", ("GPU",)
        )
...
def test_upsert_updates_integer(
    db_connection: psycopg.Connection, single_product_score_comment: ProductScore
) -> None:
    ...
    with db_connection.cursor() as cursor:
        ...
        cursor.execute(
            "SELECT mention_count FROM product_rankings WHERE product_topic = %s;", ("GPU",)
        )
        result = cursor.fetchone()
        assert result is not None
        assert result[0] == 200
...
def test_upsert_updates_booleans(
    db_connection: psycopg.Connection, single_product_score_comment: ProductScore
) -> None:
...
def test_upsert_updates_floats(
    db_connection: psycopg.Connection, single_product_score_comment: ProductScore
) -> None:
...
def test_upsert_updates_calculation_date(
    db_connection: psycopg.Connection, single_product_score_comment: ProductScore
) -> None:
    ...
    with db_connection.cursor() as cursor:
        ...
        cursor.execute(
            "SELECT calculation_date FROM product_rankings WHERE product_topic = %s;", ("GPU",)
        )
        result = cursor.fetchone()
        assert result is not None
...
def test_transaction_rollback_on_error(
    ...
    with db_connection.cursor() as cursor:
        cursor.execute(
            "SELECT COUNT(*) FROM product_rankings WHERE product_topic = %s;", ("Animals",)
        )
        count = cursor.fetchone()
        assert count is not None
        assert count[0] == 0
...
def test_same_product_name_with_different_time_windows(
    db_connection: psycopg.Connection, single_product_score_comment: ProductScore
) -> None:
    ...
    with db_connection.cursor() as cursor:
        ...
        cursor.execute(
            "SELECT time_window FROM product_rankings WHERE product_topic = %s;", ("GPU",)
        )
...
def test_mixed_batch_with_duplicates(
    ...
    batch_upsert_product_score(
        db_connection, [original_comment, duplicate_one, duplicate_two, new_one, new_two]
    )
    ...
    with db_connection.cursor() as cursor:
        ...
        cursor.execute(
            "SELECT mention_count, product_name FROM product_rankings WHERE product_topic = %s;",
            ("GPU",),
        )
        results = cursor.fetchall()
        assert results is not None
