...
class XgBoostModel:
    ...
    def run(self, file_path: str) -> None:
        enc_labels, enc_corpus, corpus, labels = load_data(
            filepath=file_path, label_encoder=self.label_encoder, embeddings_model=self.model
        )

        train_x, train_y, test_x, test_y, test_id = split_dataset(
            encoded_corpus=enc_corpus, encoded_labels=enc_labels, labels=labels
        )
...
def check_categories(csv_path: str) -> None:
    ...
    if df["category"].nunique() < 2:
        ...
        print(
            "Training will fail with stratification or classification metrics that require multiple classes."
        )
    else:
