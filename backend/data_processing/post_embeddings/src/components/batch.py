...
def batch(
    ...
    batch_size: int,
    max_wait: float = 2.0
) -> Generator[list[str], list[str], list[QueueMessage]]:
    ...
    while True:
        try:
            ...
            if batch and (len(batch) >= batch_size or (batch_start and time.time() - batch_start >= max_wait)):
                ...
                post_bodies = [extract_post_body(msg_body) for msg_body in postMsgs] # extract sentences only from Raw post
