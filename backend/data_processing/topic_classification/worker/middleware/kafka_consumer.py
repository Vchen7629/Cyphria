...
class KafkaConsumer:
    def __init__(
        ...
        try:
            ...
            logger.error(
                event_type="Kafka",
                message=f"Create Consumer Error: {e}"
            )
            raise

    def poll_for_new_messages(self, timeout_ms: int = 1000) -> tuple[dict[str, str]]:
        batch = [] # batch of msgs to be processed
        ...
        while len(batch) < 10 and time.time() - start < 1.0:
            try:
                ...
                self.structured_logger.error(
                    event_type="Kafka",
                    message=f"Error consumer polling messages: {e}"
                )
                return []
