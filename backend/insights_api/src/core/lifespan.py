...
async def lifespan(app: FastAPI) -> AsyncGenerator[Any, Any]:
    ...
    try:
        ...
        logger.info(
            event_type="insights_api startup", message="Database health check passed"
        )
    except Exception as e:
        logger.error(
            event_type="insights_api startup",
    ...
    try:
        ...
        cache_avail = True
        logger.info(
            event_type="insights_api startup", message="Valkey health check passed"
        )
    except Exception as e:
        logger.warning(
            event_type="insights_api startup",
            message=f"Valkey health check failed: {e}",
        )

    logger.info(
        event_type="insights_api startup", message="Initialized cache (valkey) client"
    )
    ...
    logger.info(
        event_type="insights_api shutdown", message="Shutting down insights api"
    )
    await engine.dispose()
    logger.info(
        event_type="insights_api shutdown", message="Database connection pool closed"
    )
