...
async def test_non_existing_product_name(
    ...
    response = await fastapi_client.client.get("/api/v1/product/sentiment_scores?product_name=auuugh&time_window=all_time")

    assert response.status_code == 404
    data = response.json()
    assert data["detail"] == "Sentiment counts not found for product: auuugh time_window: all_time"
...
async def test_correct_time_window(
    ...
    products = [
        {**single_product_ranking_row, "product_name": "product_1", "positive_count": 50, "time_window": "90d"},
        {**single_product_ranking_row, "product_name": "product_1", "positive_count": 55, "time_window": "all_time"},
    ]
    ...
    product_res = await fastapi_client.client.get(f"/api/v1/product/sentiment_scores?product_name=product_1&time_window={time_window}")
