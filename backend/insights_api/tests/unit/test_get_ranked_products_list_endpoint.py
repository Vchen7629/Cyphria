...
def test_invalid_product_topic_param(mock_fastapi: FastAPI, product_topic: str | None) -> None:
    ...
    with TestClient(mock_fastapi) as client:
        if product_topic is None:
            ...
            response = client.get(f"/api/v1/topic/products?product_topic={product_topic}&time_window=all_time")
...
def test_invalid_time_window_param(mock_fastapi: FastAPI, time_window: str | None) -> None:
    ...
    with TestClient(mock_fastapi) as client:
        if time_window is None:
            ...
            response = client.get(f"/api/v1/topic/products?product_name=huuuh&time_window={time_window}")
...
def test_no_ranked_products(mock_fastapi: FastAPI) -> None:
    ...
    with patch("src.routes.topic.fetch_products", return_value=None):
        with TestClient(mock_fastapi) as client:
            response = client.get("/api/v1/topic/products?product_topic=huuuh&time_window=all_time")

            assert response.status_code == 404
            assert response.json()["detail"] == "No products fetched for the topic: huuuh"