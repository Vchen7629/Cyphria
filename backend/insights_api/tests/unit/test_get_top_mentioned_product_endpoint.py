...
def test_invalid_product_topic_param(
    ...
    with TestClient(mock_fastapi) as client:
        if product_topic is None:
            ...
            response = client.get(
                f"/api/v1/category/topic_most_mentioned_product?product_topic={product_topic}"
            )
...
def test_no_top_products(mock_fastapi: FastAPI) -> None:
    ...
    with patch(
        "src.routes.category.fetch_topic_top_mentioned_products", return_value=[]
    ):
        with TestClient(mock_fastapi) as client:
            response = client.get(
            ...
            assert (
                ...
                == "top products not found for product_topic: GPU"
            )
