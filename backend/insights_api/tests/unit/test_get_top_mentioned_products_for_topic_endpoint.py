...
def test_invalid_category_param(mock_fastapi: FastAPI, category: str | None) -> None:
    ...
    with TestClient(mock_fastapi) as client:
        if category is None:
            ...
            response = client.get(
                f"/api/v1/category/top_mentioned_products?category={category}"
            )
...
def test_no_category_topics(mock_fastapi: FastAPI) -> None:
    ...
    with (
        patch("src.routes.category.get_topics_for_category", return_value=[]),
        patch("src.routes.category.get_cache_value") as mock_get_cache,
    ):
        with TestClient(mock_fastapi) as client:
            response = client.get(
                "api/v1/category/top_mentioned_products?category=gaming"
            )
...
def test_no_top_mentioned_products(mock_fastapi: FastAPI) -> None:
    ...
    with (
        patch("src.routes.category.fetch_top_mentioned_products", return_value=[]),
        ...
        with TestClient(mock_fastapi) as client:
            ...
            assert (
                ...
                == "top products not found for category: gaming"
            )
            mock_get_cache.assert_not_called()
