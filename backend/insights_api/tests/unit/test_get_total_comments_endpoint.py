...
def test_invalid_product_topic_param(mock_fastapi: FastAPI, product_topic: str | None) -> None:
    ...
    with TestClient(mock_fastapi) as client:
        if product_topic is None:
            ...
            response = client.get(f"/api/v1/topic/total_comments?product_topic={product_topic}&time_window=all_time")
...
def test_invalid_time_window_param(mock_fastapi: FastAPI, time_window: str | None) -> None:
    ...
    with TestClient(mock_fastapi) as client:
        if time_window is None:
            ...
            response = client.get(f"/api/v1/topic/total_comments?product_topic=huuuh&time_window={time_window}")
