...
class MultimodalNaiveBayesModel:
    def __init__(self):
        ...
        self.vectorizer = TfidfVectorizer(ngram_range=(1, 2), token_pattern=token_pattern, min_df=3) 
    ...
    def preprocess(self):
        ...
        self.features = selector.fit_transform(X, y)
    ...
    def evaluate(self):
        ...
        print(classification_report(self.Y_test, nb_y_pred))

        print("Naive SVC Classification Report:")
        print(classification_report(self.Y_test, svm_y_pred))

        print("Accuracy of naive bayes model: %.2f%%" % ((nb_correct / float(len(nb_y_pred)) * 100)))
        nb_roc_auc_ovo = roc_auc_score(self.Y_test, nb_y_score, multi_class='ovr', average='macro')
        print("AUC ROC Score for Naive Bayes OVO: %.2f%%" % nb_roc_auc_ovo)
        print("Accuracy of support vector machine model: %.2f%%" % ((svm_correct / float(len(svm_y_pred)) * 100)))
