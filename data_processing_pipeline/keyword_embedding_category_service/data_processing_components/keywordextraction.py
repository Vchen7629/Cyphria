...
class KeywordExtraction:
    ...
    def Extraction(
        ...
        if input_batch_size == 1:
            if len(raw_keybert_output) > 0 and isinstance(
                raw_keybert_output[0],
                (
                    list,
                    tuple,
                ),
            ...
            elif len(raw_keybert_output) == 1 and isinstance(
                raw_keybert_output[0],
                list,
            ):
                ...
                print(
                    f"WARN: Unexpected output format for single document. Output: {raw_keybert_output}. Returning empty list for doc."
                )
                keywords_parsed.append([])
            ...
            for doc_result in raw_keybert_output:
                if isinstance(
                    ...
                    print(
                        f"WARN: Expected list for doc results, got {type(doc_result)}. Appending empty list."
                    )
                    keywords_parsed.append([])
        else:
            print(
                f"ERROR: KeyBERT output length ({len(raw_keybert_output)}) does not match input size ({input_batch_size}). Returning Nones."
            )
            return pd.Series(
                [None] * input_batch_size,
                index=original_index,
            )

        if len(keywords_parsed) != input_batch_size:
            print(
                f"CRITICAL ERROR: keywords_parsed length ({len(keywords_parsed)}) mismatch with input size ({input_batch_size}) before Series creation. Problematic raw output: {raw_keybert_output}. Returning Nones."
            )
            return pd.Series(
                [None] * input_batch_size,
                index=original_index,
        ...
        print(
            f"Keyword extraction batch (size {input_batch_size}) finished in: {batch_duration:.4f} seconds."
        )
