...
from data_processing_components import (
    keywordextraction,
...
class TransformData:
    ...
    def poll_for_new_messages(
        ...
        try:
            ...
            print(
                f"WARN: Error processing record value offset {record.offset} in {tp}: {e}. Skipping record."
            )
            yield None
    ...
    def process_data(
        ...
        start = time.time()
        reddit_post_dataframe = pd.DataFrame(
            data=extracted_data
        )  # create initial dataframe with raw reddit post data
        try:
            keyword_series = self.keyword_extractor.Extraction(reddit_post_dataframe["body"])
            vector_embedding_series = self.vector_embedder.Generate_Vector_Embeddings(
                reddit_post_dataframe["body"]
            )
            category_embeddings = self.category_classifier.Classify_Category(
                vector_embedding_series
            )
