import json, time
from data_processing_components.aspect_based_sentiment_analysis import Aspect_Based_Sentiment_Analysis
import pandas as pd

class TransformData:
    def __init__(self, consumer_instance, tokenizer, model):
        try:
            self.instance = consumer_instance
            self.Aspect_Instance = Aspect_Based_Sentiment_Analysis(tokenizer=tokenizer, model=model)
    ...
    def Transform_Data(self, data):
        ...
        try:
            ...
            required_cols = ['body', 'keywords']
            if not all(col in dataframe.columns for col in required_cols):
                print(f"ERROR: Input data missing required columns. Found: {dataframe.columns}. Required: {required_cols}")
                return None

            sentence_keyword_pair = [
                (index, sentence, keyword)
                for index, sentence, keyword_list in zip(dataframe.index, dataframe['body'], dataframe['keywords'])
            ...
            if not sentence_keyword_pair:
                ...
                dataframe['keyword_sentiments'] = pd.Series([{} for _ in range(len(dataframe))], index=dataframe.index)
                return dataframe
            ...
            start2 = time.time()
            sentiment_df = pd.DataFrame(sentiment_results, columns=['index', 'keyword', 'sentiment'])
